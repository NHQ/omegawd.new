...
function Writer(e, file){
     		...
     		var winX = win.winX
				...
				$('#box').fadeOut(350);
				$('body').empty().prepend('<style>'+ omg.css['options.css'] + omg.css['writer.options.css'] + omg.css['writer.css'] + '</style>').append(omg.jade('writer', {getSVG: win.getSVG}))
				tempLoaded()
		...
		//							$('#editor').empty();
					        ...
					        // All google fonts JSON = https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyCC3EWAlPbKZ1dYeq9zNovCmRh35PL2GQQ
									...
									// this is init stuff
					        var editor = self.writer.editor = ace.edit("editor");
