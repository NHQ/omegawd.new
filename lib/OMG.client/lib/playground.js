	...
	this.init = function (){
		...
		function next(){
			...
			self.targetView = function(obj){
				$('#toolSet').empty().append(omg.jade('elemTargetView', {selected : {css : self.selectedObject, html : self.selected}})).css('overflow', 'auto').children().find('.cssPrimitives').click(function(){
					var css = this
						,	child = $('#' + css.id + ' .cssPrimitiveVal')
					;
					switch (css.getAttribute('data-type').toLowerCase()) {
						...
						case 'float':
						  ...
						  range.attr('min', 0 - Math.abs(0 - primVal) - 50)
							 		 .attr('max', 50 + 0 + Math.abs(0 - primVal) * 2)
							 		 .val(primVal)
							 		 .unbind('change')
									 .bind('change', function(){
											var val = this.value
											;
											$(self.selected).css(css.id, parseFloat(val) + '%')
			...
			$('#stage').append(c);
			var canvas = Canvas('xyRangeCanvas', [600,600]);
