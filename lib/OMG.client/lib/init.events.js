...
exports.initFS = function(fs){
	...
	function checkUpLoad(err, res){
		...
		if (_.any(res, truthTest)){
			...
			omg.fs.mkdir('/temp', undefined, omg.errorHandler);
			omg.fs.mkdir('/draftsman', JSON.stringify(metadb), undefined, omg.errorHandler);
			omg.fs.mkdir('/publications', JSON.stringify(metadb), undefined, omg.errorHandler);
			omg.fs.mkdir('/media', JSON.stringify(metadb), undefined, omg.errorHandler);
