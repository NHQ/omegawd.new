var win = require('./client.utils.js')
	...
	, templates = ['container', 'textarea', 'shape', 'list', 'quote', 'link', 'paragraph', 'header', 'pre-', 'canvas', 'iframe', 'media', 'HTML', 'input', 'form', 'button', 'slider']
	...
	,	layouts = ['absolute', 'scrollY', 'scrollX', 'scrollXY', 'freestyle']
...
var playground = function(){
	...
	this.init = function (){
		...
		,		dataURI = omg.fs.readDataUrl(blob, function(e,res){
										...
										$('body').append(omg.jade('playground', {getSVG: win.getSVG, elemenos : templates, layouts: layouts}))
										next.call(self)
...
//			var animationFrame = window.webkitRequestAnimationFrame;
			...
			self.targetView = function(obj){
				...
				var elementStyleMenu = element.style(self.selectedObject)
				
				var options = $('#toolSet').empty().append(omg.jade('elemTargetView', {compute : win.getPropVal, selected : {css : self.selectedObject, elem : self.selected}, parent : self.selectedParent}));
