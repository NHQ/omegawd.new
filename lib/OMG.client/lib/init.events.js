...
exports.initFS = function(fs){
	...
	function checkUpLoad(err, res){
		...
		if (_.any(res, truthTest)){
			...
			omg.fs.mkdir('/draftsman', JSON.stringify(metadb), undefined, null);
			omg.fs.mkdir('/publications', JSON.stringify(metadb), undefined, null);
			omg.fs.mkdir('/media', JSON.stringify(metadb), undefined, null);
